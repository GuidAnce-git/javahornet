...
public class NewHope {
    ...
    public void keygen(byte[] send, int sendOffset) {
        ...
        try {
            randombytes(seed);
            sha3256(seed, 0, seed, SEEDBYTES); /* Don't send output of system RNG */
    ...
    public void sharedb(byte[] sharedkey, int sharedkeyOffset,
        ...
        try {
            ...
            helprec(c, v, noiseseed);

            encode_b(send, sendOffset, bp, c);

            rec(skey, v, c);

            sha3256(sharedkey, sharedkeyOffset, skey, 32);
        } finally {
            sp.destroy();
            ep.destroy();
            v.destroy();
    ...
    private static void decode_b(Poly b, Poly c, byte[] r, int roffset) {
        ...
        for (i = 0; i < PARAM_N / 4; i++) {
            c.coeffs[4 * i] = (char) (r[POLY_BYTES + roffset + i] & 0x03);
    ...
    private static class Poly {
        ...
        public void frombytes(byte[] a, int offset) {
            int i;
            for (i = 0; i < PARAM_N / 4; i++) {
                coeffs[4 * i] = (char) ((a[offset + 7 * i] & 0xff) | ((a[offset + 7 * i + 1] & 0x3f) << 8));
        ...
        public void tobytes(byte[] r, int offset) {
            ...
            int c;
            for (i = 0; i < PARAM_N / 4; i++) {
                t0 = barrett_reduce(coeffs[4 * i]); //Make sure that coefficients have only 14 bits
    ...
    private static void helprec(Poly c, Poly v, byte[] seed) {
        int[] v0 = new int[8];
        ...
        try {
            crypto_stream_chacha20(rand, 0, 32, ((long) (byte) 3) << 56, seed);

            for (i = 0; i < 256; i++) {
                rbit = (rand[i >> 3] >> (i & 7)) & 1;

                k = f(v0, 0, v0, 4, 8 * v.coeffs[i] + 4 * rbit);
                ...
                c.coeffs[i] = (char) ((v_tmp0 - v_tmp3) & 3);
    ...
    private static void rec(byte[] key, Poly v, Poly c) {
        ...
        for (i = 0; i < 256; i++) {
            ...
            tmp0 = 16 * PARAM_Q + 8 * (int) v.coeffs[i] - PARAM_Q * (2 * c.coeffs[i] + c768);
    ...
    static void shake128_absorb(long[] s, byte[] input) {
        keccak_absorb(s, SHAKE128_RATE, input, 0, NewHope.SEEDBYTES, (byte) 0x1F);
    }

    static void shake128_squeezeblocks(byte[] output, int nblocks, long[] s) {
        keccak_squeezeblocks(output, 0, nblocks, s, SHAKE128_RATE);
    }

    private static final int SHA3_256_RATE = 136;

    private static void sha3256(byte[] output, int outputOffset, byte[] input, int inputByteLen) {
        long[] s = new long[25];
        byte[] t = new byte[SHA3_256_RATE];
        int i;

        try {
            keccak_absorb(s, SHA3_256_RATE, input, 0, inputByteLen, (byte) 0x06);
    ...
    private static void crypto_core_chacha20(byte[] out, int outOffset, long nonce, int blknum, byte[] k) {
        ...
        store_littleendian(out, outOffset, x0);
    ...
    private static void crypto_stream_chacha20(byte[] c, int coffset, int clen, long n, byte[] k) {
        ...
        if (clen != 0) {
            ...
            try {
                ...
                if (clen >= 0) {
                    System.arraycopy(block, 0, c, coffset, clen);
                }
            } finally {
